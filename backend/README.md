...
- Para ver o trigger de Mensagem crie uma nova conversa, mande a primeira mensagem e recarregue a página.
...
Uma delas foi a organização de diretórios e rotas por versão, para não acabar prejudicando usuários que utilizam uma versão antiga quando uma nova surgir, visando isso criei uma pasta para guardar todas as versões da API ("backend/chat/api/") aqui estará as pastas com todas as versões que o sistema vier a atualizar.
